...
const JSON_DB_PATH = path_1.default.join(__dirname, '../database.json');
...
function loadJsonDb() {
    if (!fs_1.default.existsSync(JSON_DB_PATH)) {
        const dbData = { tasks: [] };
        fs_1.default.writeFileSync(JSON_DB_PATH, JSON.stringify(dbData, null, 2), 'utf8');
        return dbData;
    }
    try {
        const fileContent = fs_1.default.readFileSync(JSON_DB_PATH, 'utf8');
...
exports.db = {
    task: {
                        ...
                        if (where[key] !== undefined) {
                            if (key === 'status' && where.status) {
                                if (where.status.in && Array.isArray(where.status.in)) {
                                    if (!where.status.in.includes(task.status))
                                        return false;
                                }
                                ...
                                if (task.energyLevel !== where.energyLevel)
                                    return false;
                            }
        ...
        findUnique: async (args) => {
            ...
            const dbData = loadJsonDb();
            const task = dbData.tasks.find(t => t.id === args.where.id && (!args.where.userId || t.userId === args.where.userId));
            if (!task)
                return null;
            return {
                ...task,
        ...
        update: async (args) => {
            ...
            const dbData = loadJsonDb();
            const taskIndex = dbData.tasks.findIndex(t => t.id === args.where.id && (!args.where.userId || t.userId === args.where.userId));
            if (taskIndex === -1) {
                throw new Error(`Record to update not found: ${args.where.id}`);
            }
