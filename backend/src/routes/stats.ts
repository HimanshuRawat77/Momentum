...
router.get('/', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    completedToday.forEach(task => {
      ...
      else if (task.priority === 'LOW') points += 5;

      if (task.energyLevel === 'HIGH') points += 10;
      else if (task.energyLevel === 'MEDIUM') points += 5;
    ...
    allTasks.forEach(task => {
      const level = task.energyLevel as 'LOW' | 'MEDIUM' | 'HIGH';
